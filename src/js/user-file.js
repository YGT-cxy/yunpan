import iconCss from './../less/icon-file.less';  // 文件图标样式
import font from  './../less/font.less';  // 字体样式
import css from './../less/user-file.less';  // 本页面的样式
import $ from 'jquery';  // 引入jquery
import WebUploader from './../lib/webuploader-0.1.5/dist/webuploader.js';  // 引入webuploader上传文件插件
import {errorCode} from './errorCode.js';  // 错误码
...
function applyDataTemplate(data) {
    ...
    for(var i = data.length - 1; i >= 0; i--) {
        txtHtml  += `<tr class="file-${data[i].type}" data-type="${data[i].type}" data-basename="${data[i].basename}" data-extension="${data[i].extension}" data-json='${JSON.stringify(data[i])}'>
...
$('#file tbody').on('contextmenu', 'tr', function(e) {
    ...
    menu.css({'top': top, 'left': e.clientX}).show()  // 显示菜单栏
        ...
        .attr('data-extension', $this.attr('data-extension'));  // 给子元素添加数据
        ...
        default:  // 删除
            if(type === 'dir') {  // 删除目录
                showModal('删除文件', '是否删除' + basename + ' 文件夹', function() {
                    new FileOrDir().delDir(basename);
                });
            } else {  // 删除文件
                showModal('删除文件', '是否删除' + basename + ' 文件', function() {
                    new FileOrDir().delFile(basename);
                });
            }
    }
});
    ...
    switch(operation) {
        ...
        case 'create_dir': // 创建文件夹
            ...
            if(!dir_name) {
                ...
                showModal('创建文件', '是否要创建' + dir_name +'文件夹', function() {
        ...
        case 'select':  // 下拉模态框
            msg.html('');
            showSelect($(e.target));  // 传入JQ对象类型的当前下拉框元素
            break;
        default:
            console.log('没有定义方法');
    }
...
function showSelect(target) {
    var parent = target.parent(),
        ...
        nextElemnt = `<div class="select-model">
                        <ul>
                            <li>请选择类型</li>
                            <li>TXT</li>
                            <li>PHP</li>
    ...
    parent.addClass('file-select-focus').after(nextElemnt);
